...
| | |
|---|---|
| **More than one database engine** | Postgres and SQLite are tested end to end; MySQL/MariaDB and SQL Server are wired in through the same adapter. A database is registered from the admin console, not in code. |
...
certificate for it on its own. The Keycloak realm file (`deploy/keycloak/`) lists `https://localhost` as
the console's redirect address, so add yours there as well.
...
Postgres and SQLite, the OAuth flow against a fake identity provider, and the admin API over HTTP. The
browser tests sign in through Keycloak's real login page.
...
docs/           design notes for the console
tests/          Python tests for both services
```
...
MySQL and SQL Server work through the same adapter, but they are not part of the automated tests yet.
The MySQL driver is included in the image by default. SQL Server also needs the Microsoft ODBC driver
installed in the image, which isn't done for you.
